      ...
      {guideOpen && (
        ...
        >
          <motion.div
            ...
            className="glass-card w-full max-w-md p-6 border border-border shadow-2xl bg-card text-foreground"
            onClick={e => e.stopPropagation()}
          >
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="font-heading font-bold text-lg text-slate-900 dark:text-white">Add {siteConfig.shortName} to your device</h3>
              ...
              <button
                type="button"
                onClick={() => setGuideOpen(false)}
                className="p-1.5 rounded-lg text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-white"
                aria-label="Close"
              >
...
function GuideBlock({ title, steps }: { title: string; steps: string[] }) {
  return (
    <div className="glass rounded-xl p-4 border border-border bg-card/50">
      <p className="text-sm font-semibold text-slate-900 dark:text-white mb-2">{title}</p>
      <ol className="space-y-2">
        {steps.map((step, i) => (
          <li key={step} className="flex gap-2 text-xs text-slate-600 dark:text-slate-400">
