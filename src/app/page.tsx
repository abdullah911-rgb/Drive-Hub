...
function LandingContent() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [carsRes, companiesRes, meRes] = await Promise.all([
          fetch('/api/cars?status=APPROVED'),
          fetch('/api/companies?status=APPROVED'),
  ...
  const stats = [
    ...
    { value: `${brands.length}+`, label: 'Car Brands' },
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden flex flex-col">
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-accent/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="relative z-10 flex flex-col min-h-screen pt-16">
            ...
            {!loading && !user && (
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-12">
                <button
                  ...
                  className="btn-ghost px-6 py-3 text-sm font-semibold rounded-xl"
                >
                  Sign In
                </button>
                <button
        ...
        {/* CTA for customers to become companies */}
        {!loading && (!user || (user.role === 'CUSTOMER' && user.status === 'APPROVED')) && (
